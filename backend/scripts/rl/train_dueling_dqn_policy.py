...
from tutor.policy.rl.dueling_dqn_policy import DuelingDQNPolicy, ReplayBuffer
from tutor.policy.rl.state_action_space import STATE_FEATURES, action_count
...
MODEL_PATH = Path("models/rl/dueling_dqn_policy.pt")
META_PATH = Path("models/rl/dueling_dqn_policy_meta.json")
JSON_REPORT = Path("evaluation_outputs/json/dueling_dqn_training_report.json")
MD_REPORT = Path("evaluation_outputs/reports/dueling_dqn_training_report.md")
...
def _load_arrays() -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]:
    ...
    states = df[STATE_FEATURES].astype(float).to_numpy(dtype=np.float32)
    next_states = df[[f"next_{feature}" for feature in STATE_FEATURES]].astype(float).to_numpy(dtype=np.float32)
...
def _evaluate(policy: DuelingDQNPolicy, df: pd.DataFrame) -> dict:
...
def train() -> dict:
    ...
    policy = DuelingDQNPolicy(state_dim=len(STATE_FEATURES), action_dim=action_count(), hidden_dim=96, gamma=0.95, lr=0.001)
    losses: list[float] = []
    epochs = 40
    batch_size = 64
    ...
    report = {
        ...
        "training_mode": "dueling_dqn_from_scratch_offline_replay",
        "loss_history": losses,
...
def main() -> None:
    ...
    print("MODULE: dueling_dqn_training")
