...
from tutor.evaluation.output_prediction_evaluator import (
    evaluate_output_predictions_from_assessment,
)
...
def main() -> None:
    ...
    print("\nOUTPUT PREDICTION EVALUATOR TEST")
    ...
    for profile in ["strong", "average", "weak", "debug_weak", "low_confidence"]:
        ...
        output = evaluate_output_predictions_from_assessment(
            assessment_output=assessment_output,
        ...
        print(
            ...
            output.get("output_prediction_question_count"),
        )
        print("overall_score:", output.get("overall_score"))
        print("quality_label:", output.get("quality_label"))
        print("dominant_output_error_type:", output.get("dominant_output_error_type"))
        print("output_error_type_counts:", output.get("output_error_type_counts"))

        for item in output.get("results", []):
            print("question_id:", item.get("question_id"))
            print("output_scores:", item.get("output_scores"))
            print("output_error_type:", item.get("output_error_type"))
            print("feedback:", item.get("feedback"))

        assert output["status"] == "success"
        assert output.get("output_prediction_question_count", 0) >= 1

    print("\nSTATUS: success")
    print("MODULE: output_prediction_evaluator")
